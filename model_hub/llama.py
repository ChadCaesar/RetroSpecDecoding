...
class LlamaModel(LLM):
    ...
    def init_model(self):
        hf_llama = LlamaForCausalLM.from_pretrained(self.local_path, torch_dtype=self.dtype)
        ...
        if self.attention_type == 'Full_Flash_Attn':
            ...
            if specdecoder_config['gpu_only'] == False:   # Offload version
                self.kv_cache = specdecoder_cache(
                    ...
                    cluster_index_mode = specdecoder_config["cluster_index_mode"],
                    cluster_index_path = specdecoder_config["cluster_index_path"],
                    fingerprint = specdecoder_config["fingerprint"],
                    prefill_bsz = self.prefill_bsz,
                    num_gpus = self.num_gpus,
                    model_size = int(re.search(r'(\d+)[B]', self.model_name).group(1)),
                )
